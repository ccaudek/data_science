...
Per i dati di @zetschefuture2019 la funzione di verosimiglianza corrisponde alla funzione binomiale di parametro $\theta \in [0, 1]$ sconosciuto. Abbiamo osservato 23 successi, $y = 23$, in 30 prove, $n = 30$.
...
Per costruire la funzione di verosimiglianza dobbiamo applicare l'@eq-likebino23 tante volte, cambiando ogni volta il valore $\theta$, ma tenendo sempre costante il valore dei dati.

Nella simulazione considereremo 100 possibili valori $\theta \in [0, 1]$.
...
L'@eq-gaussian-sim-like dipende dai parametri $\mu$ e $\sigma$ e dai dati $y$. Per semplicità, ipotizzeremo $\sigma$ noto e uguale a 15. Nella simulazione considereremo 1000 valori $\mu$ compresi tra 70 e 160.

```{r}
mu <- seq(70, 160, length.out = 1e3)
```

Dato che consideriamo 1000 possibili valori $\mu$, per costruire la funzione di verosimiglianza applicheremo l'@eq-gaussian-sim-like 1000 volte. In ciascun passo della simulazione inseriremo nell'@eq-gaussian-sim-like

-   il singolo valore $y$ considerato,
-   il valore $\sigma$ assunto noto,
-   ciascuno dei valori $\mu$ che abbiamo considerato.

La distribuzione Gaussiana è implementata in $\mathsf{R}$ nella funzione `dnorm()`. La funzione `dnorm()` ha tre argomenti:

-   il valore $y$ (o il vettore $y$),
-   la media, ovvero il parametro $\mu$,
-   la deviazione standard, ovvero il parametro $\sigma$.

Usando la funzione `dnorm()` 1000 volte, una volta per ciascuno dei valori $\mu$ che abbiamo considerato, otteniamo 1000 coppie di valori $\mu$ e $f(\mu)$. La curva che interpola tali punti è la funzione di verosimiglianza.
...
La moda di questa funzione di verosimiglianza è 114.
